...
router.post("/createApartmentPost", ensureAuth, async (req, res) => {
  ...
  try {
    ...
    const postRange = extreamsToRange(
      req.body[dbNames.S_Year],
      req.body[dbNames.E_Year]
    );
...
router.post("/getBuildings", ensureAuth, async (req, res) => {
  try {
    ...
    if (!req.body.StreetId) {
      throw `missing fields: ${dbNames.ST_Id}`;
    }
    ...
    console.log(street.DB_Buildings);
...
router.post("/getApartments", ensureAuth, async (req, res) => {
  try {
    if (!req.body.buildingId) {
      console.log(req.body[dbNames.BU_Id]);
      throw `missing fields: ${dbNames.BU_Id}`;
    }
...
router.post("/getApartmentPosts", ensureAuth, async (req, res) => {
  try {
    if (!req.body[dbNames.APA_Id]) {
      throw `missing fields: ${dbNames.APA_Id}`;
    }
    let DB_ApartmentPosts = await APA_Post.find(
      { APA_Id: req.body[dbNames.APA_Id] },
      {
        _id: 0,
        CreatorsGoogleID: 0,
        // Post_Id:1,
    ...
    console.log(DB_ApartmentPosts);
    res.json(DB_ApartmentPosts);
    // const DBInfo = { ...apartmentPostReqData };
    // DBInfo.Post_Id = ["PostId"];
    // console.log(hideDB_Names(DB_ApartmentPosts, DBInfo));
...
router.post("/getBuildingPosts", ensureAuth, async (req, res) => {
  try {
    if (!req.body[dbNames.BU_Id]) {
      throw `missing fields: ${dbNames.BU_Id}`;
    }
    let DB_BuildingPosts = await BU_Post.find(
      { BU_Id: req.body[dbNames.BU_Id] },
      {
        _id: 0,
        CreatorsGoogleID: 0,
        // Post_Id: 1,
...
router.post("/deleteApartmentPost", ensureAuth, async (req, res) => {
  try {
    if (!req.body[dbNames.Post_Id]) {
      throw `missing fields: ${dbNames.Post_Id}`;
    }
    await authorizeReq(req, APA_Post, false);
    await APA_Post.findOneAndDelete({ Post_Id: req.body.IdOfPost });
    await User.update(
      { User_Id: req.user.User_Id },
      { $pull: { activeYears: { PostId: req.body.IdOfPost } } },
      { multi: true }
    );
    res.json({ id: req.body.IdOfPost });
  } catch (error) {
...
const authorizeReq = async (req, postModule, checkYearAlso) => {
  ...
  if (checkYearAlso) {
    ...
    if (oldPost) {
      error += "cant update posts from last year";
    }
  }
  if (error != "") {
    throw error;
...
const validateMissingFields = (req, reqInfo) => {
  for (const [key, value] of Object.entries(reqInfo)) {
    if (req.body[value[0]] == null) {
      if (req.session.missings) {
        req.session.missings += ` ${value[0]} ,`;
        continue;
      }
...
const validateFieldsFormat = (req, reqInfo) => {
  for (const [key, value] of Object.entries(reqInfo)) {
    try {
      ...
      if (req.session.formatIssuse) {
        req.session.formatIssuse += ` ${value[0]} : ${error} ,`;
        continue;
      }
  ...
  if (req.session.formatIssuse) {
    throw req.session.formatIssuse;
  }
};
...
const createObjectForDB = (req, reqInfo) => {
  ...
  for (let [key, value] of Object.entries(reqInfo)) {
    if (value[2]) newObject[key] = value[2](req.body[value[0]]);
  }
